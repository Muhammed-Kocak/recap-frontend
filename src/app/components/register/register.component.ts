...
export class RegisterComponent implements OnInit {
  ...
  register(){
    if(this.registerForm.valid){
      ...
      this.authService.register(registerModel).subscribe(respose=>{
        this.toastrService.success(respose.message);
        localStorage.setItem("token",respose.data.token);
        this.router.navigate(["login"]);
      },responseError=>{
        console.log(responseError)
        this.toastrService.error(responseError.error,"Bir şeyler ters gitti");
      })
    }
  }
}
